...
func TestSessionCreation_success(t *testing.T) {
	...
	mockedIAMBluemixSessionRequest := &MockedRequest{
		Method:     "POST",
	...
	mockedIAMBluemixSessionResponse := &MockedResponse{
		StatusCode:  200,
		ContentType: "application/json",
		Body: map[string]interface{}{
			"access_token":      "access",
	...
	transactionsIAM := []*MockedTransaction{
		...
		{
			Request:  mockedIAMBluemixSessionRequest,
			Response: mockedIAMBluemixSessionResponse,
		},
	}
	...
	resetEnv := setEnv(map[string]string{
		"IBMCLOUD_IAM_TOKEN":        "",
		"IBMCLOUD_IAM_API_ENDPOINT": c.IAMEndpoint,
	}, t)
	defer resetEnv()
	...
	if e != nil {
		t.Fatalf("Session creation failed %v", e)
	}
