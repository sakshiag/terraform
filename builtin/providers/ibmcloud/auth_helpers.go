...
func newSession(c *Config) (*Session, error) {
	...
	uaaToken := os.Getenv("IBMCLOUD_UAA_TOKEN")
	iamRefreshToken := os.Getenv("IBMCLOUD_IAM_REFRESH_TOKEN")
	uaaRefreshToken := os.Getenv("IBMCLOUD_UAA_REFRESH_TOKEN")
	// ibmid/ibmid_password or iam token needs to be provided
	// iamtoken, uaaToken, iamRefreshToken and  uaaRefreshToken are meant to be used internally at this point by IBM cloud hence don't
	// show that in error to the regular user of terraform
	if (c.IBMID == "" || c.IBMIDPassword == "") && (iamToken == "" || uaaToken == "" || iamRefreshToken == "" || uaaRefreshToken == "") {
	...
	ibmcloudSession.SoftLayerIMSToken = imstoken
	ibmcloudSession.SoftLayerUserID = imsuserid

	slUsername, slAPIKey, err := fetchSoftLayerAPIKey(c, imsuserid, imstoken)
	if err != nil {
		return nil, err
	}
	ibmcloudSession.SoftLayerUserName = slUsername
	ibmcloudSession.SoftLayerAPIKey = slAPIKey
	softlayerSession := &slsession.Session{
