...
func resourceIBMCloudInfraUser() *schema.Resource {
	return &schema.Resource{
		Create:   resourceIBMCloudInfraUserCreate,
		Read:     resourceIBMCloudInfraUserRead,
		Update:   resourceIBMCloudInfraUserUpdate,
		Delete:   resourceIBMCloudInfraUserDelete,
		Exists:   resourceIBMCloudInfraUserExists,
...
func resourceIBMCloudInfraUserCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceIBMCloudInfraUserRead(d, meta)
}

func resourceIBMCloudInfraUserRead(d *schema.ResourceData, meta interface{}) error {
...
func resourceIBMCloudInfraUserUpdate(d *schema.ResourceData, meta interface{}) error {
	sess := meta.(ClientSession).SoftLayerSession()
	service := services.GetUserCustomerService(sess)
	...
	if err != nil {
		return fmt.Errorf("Error received while editing ibmcloud_infra_user: %s", err)
	}
	...
	if d.HasChange("permissions") {
		...
		if err != nil {
			return fmt.Errorf("Error received while removing old permissions from ibmcloud_infra_user: %s", err)
		}

		// 'add' new permission set
		_, err = service.AddBulkPortalPermission(newPermissions)
		if err != nil {
			return fmt.Errorf("Error received while assigning new permissions to ibmcloud_infra_user: %s", err)
		}
	}
	...
	if d.HasChange("has_api_key") {
		...
		if api_key_flag {
			if len(keys) == 0 { // means key does not exist, so create one.
				...
				if err != nil {
					return fmt.Errorf("Error creating API key while editing ibmcloud_infra_user resource: %s", err)
				}
			...
			if len(keys) > 0 {
				...
				if err != nil {
					return fmt.Errorf("Error deleting API key while editing ibmcloud_infra_user resource: %s", err)
				}
...
func resourceIBMCloudInfraUserDelete(d *schema.ResourceData, meta interface{}) error {
	sess := meta.(ClientSession).SoftLayerSession()
	service := services.GetUserCustomerService(sess)
...
func resourceIBMCloudInfraUserExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	service := services.GetUserCustomerService(meta.(ClientSession).SoftLayerSession())
