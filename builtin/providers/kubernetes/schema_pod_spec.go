...
func podSpecFileds() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"active_deadline_seconds": {
			Type:         schema.TypeInt,
			Optional:     true,
			Description:  "Duration in seconds the pod may be active on the node relative to StartTime before the system will actively try to mark it failed and kill associated containers.",
		...
		"dns_policy": {
			...
			ValidateFunc: validateDNSPolicy,
		},
		"host_ipc": {
		...
		"restart_policy": {
			...
			ValidateFunc: validateRestartPolicy,
		},
		...
		"image_pull_secrets": {
			...
			Description: "Optional list of references to secrets in the same namespace to use for pulling any of the images used by this PodSpec. If specified, these secrets will be passed to individual puller implementations for them to use. For example, in the case of docker, only DockerConfig type secrets are honored. More info: http://kubernetes.io/docs/user-guide/images#specifying-imagepullsecrets-on-a-pod",
			Optional:    true,
...
func volumeSchema() *schema.Resource {
	...
	v["persistent_volume_claim"] = &schema.Schema{
		Type:        schema.TypeList,
		Description: "Represents a reference to a PersistentVolumeClaim in the same namespace. More info: http://kubernetes.io/docs/user-guide/persistent-volumes#persistentvolumeclaims",
		Required:    true,
		MaxItems:    1,
		Elem: &schema.Resource{
			Schema: map[string]*schema.Schema{
				"claim_name": {
					Type:        schema.TypeString,
					Description: "The name of a PersistentVolumeClaim in the same namespace as the pod using this volume. More info: http://kubernetes.io/docs/user-guide/persistent-volumes#persistentvolumeclaims",
					Optional:    true,
				},
				"read_only": {
					Type:        schema.TypeBool,
